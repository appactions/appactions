...
const refreshNativeNode = element => {
  ...
  if (rest.length) {
    throw new Error(`Refreshing element failed, because "dataset.uniqueSelector" was not unique anymore: ${uniqueSelector}`);
  }
...
const refreshSubject = $subject => {
  const subject = Array.from($subject).map(el => {
...
const formatArguments = args => args.map(formatArgument).join(', ');
class AppActionsError extends Error {} // "instanceof Element" seems like not working reliably, using this hack for now
...
const enhanceTestableMethods = (role, methods, isSelectors) => {
  return Object.fromEntries(Object.entries(methods).map(([key, fn]) => {
    const outerEnhancedFunction = (...args) => {
      ...
      const enhancedFunction = function ($el) {
        ...
        if ($el.length === 0) {
          ...
          throw new Error(`Multiple elements were passed to ${role}.${key}, but only a single one is supported`);
        }
      ...
      enhancedFunction.displayFunctionBody = innerFunction.toString(); // functionality

      enhancedFunction.isImpure = !isSelectors;
      enhancedFunction.isTestableFunction = true; // state
...
function createTestable(config) {
...
function expectToFailWithMessage(expectedMessage) {
  Cypress.once('fail', error => {
    if (error.message !== expectedMessage) {
      throw new Error(`Test expected to fail with a certain error, but instead it failed with:\n\n\t${error.message}\n\nThe expected error is:\n\n\t${expectedMessage}`);
    }
...
const register$1 = name => {
  Cypress.Commands.add(name, {
    ...
    const getConsoleProps = value => () => ({ ...getConsolePropsWithoutResult(),
...
function getDisplayName$1(fiber) {
...
function unwrapJQuery(el) {
  if (isJquery(el)) {
    ...
    if (el.length > 1) {
      throw new Error('jQuery selector were passed to App Actions API with multiple element selected. Only one supported.');
    }
  }
...
const hasMatchingName = componentName => fiber => {
  const displayName = getDisplayName$1(fiber);
...
function callInteraction($el, methodName, ...args) {
  ...
  if (matches.length > 1) {
    throw new Error(`Found more then one instances with "${methodName}" interaction. Narrow down the selection to match exactly a single element.`);
  }

  const [fiber] = matches;
  const componentName = getDisplayName$1(fiber);
  const driver = getDriver(fiber);
  ...
  const DOMNode = Cypress.AppActions.reactApi.findNativeNodes(fiber); // return componentMap[methodName].apply(null, args).call(null, Cypress.$(DOMNode), stateNode);

  return method.apply(null, args).call(null, Cypress.$(DOMNode), stateNode);
}
function findElementByPredicate(fiber, predicate) {
...
function findElementByRole(fiber, role) {
  return findElementByPredicate(fiber, isPartOfRole(role));
}
function findElementByReactComponentName(fiber, componentName) {
  return findElementByPredicate(fiber, hasMatchingName(componentName));
}
function findAncestorElementByPredicate(fiber, predicate) {
...
function findAncestorElementByRole(fiber, role) {
  return findAncestorElementByPredicate(fiber, isPartOfRole(role));
}
function findAncestorElementByReactComponentName(fiber, componentName) {
...
})(typeof commonjsGlobal$1 != 'undefined' ? commonjsGlobal$1 : commonjsGlobal$1, function (root) {
...
var getID_1 = /*#__PURE__*/Object.defineProperty({
	getID: getID_2
}, '__esModule', {value: true});

var getClasses_2 = getClasses;
var getClassSelectors_1 = getClassSelectors;
...
var getClasses_1 = /*#__PURE__*/Object.defineProperty({
	getClasses: getClasses_2,
	getClassSelectors: getClassSelectors_1
}, '__esModule', {value: true});

var getCombinations_2 = getCombinations;
/**
...
var getCombinations_1 = /*#__PURE__*/Object.defineProperty({
	getCombinations: getCombinations_2
}, '__esModule', {value: true});

var getAttributes_2 = getAttributes;
...
function getAttributes(el) {
  ...
  var attrs = [].concat(_toConsumableArray$1(attributes));
...
var getAttributes_1 = /*#__PURE__*/Object.defineProperty({
	getAttributes: getAttributes_2
}, '__esModule', {value: true});
...
var isElement_1 = /*#__PURE__*/Object.defineProperty({
	isElement: isElement_2
}, '__esModule', {value: true});

var getNthChild_2 = getNthChild;
...
var getNthChild_1 = /*#__PURE__*/Object.defineProperty({
	getNthChild: getNthChild_2
}, '__esModule', {value: true});
...
var getTag_1 = /*#__PURE__*/Object.defineProperty({
	getTag: getTag_2
}, '__esModule', {value: true});

var isUnique_2 = isUnique;
/**
...
var isUnique_1 = /*#__PURE__*/Object.defineProperty({
	isUnique: isUnique_2
}, '__esModule', {value: true});

var getParents_2 = getParents;
...
var getParents_1 = /*#__PURE__*/Object.defineProperty({
	getParents: getParents_2
}, '__esModule', {value: true});

var getData_1 = createCommonjsModule$1(function (module, exports) {
...
exports.getData = function getData(selectorType, attributes) {
  for (var i = 0; i < attributes.length; i++) {
    // extract node name + value
    var _attributes$i = attributes[i],
        nodeName = _attributes$i.nodeName,
        value = _attributes$i.value; // if this matches our selector
...
function getAllSelectors(el, selectors, attributesToIgnore) {
  var funcs = {
    'tag': getTag_1.getTag,
    'nth-child': getNthChild_1.getNthChild,
...
function getUniqueSelector(element, selectorTypes, attributesToIgnore) {
  ...
  try {
    for (var _iterator = selectorTypes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var selector = elementSelectors[selectorType]; // if we are a data attribute

      if (dataRegex.test(selectorType)) {
        var dataSelector = (0, getData_1.getData)(selectorType, attributes); // if we found a selector via data attributes
...
const register = (name, {
  ...
  Cypress.Commands.add(name, {
    ...
    const getValue = () => {
      ...
      const anyCandidatesLoading = evaluatedCandidates.some(({
        loadingResult
      }) => {
      ...
      if (!anyCandidatesLoading) {
        result = evaluatedCandidates.filter(({
          filterResult
        }) => {
      ...
      if (result.length === 0) {
        const maybeFilterIssue = evaluatedCandidates.find(({
          filterResult
        }) => filterResult instanceof Error); // if we couldn't come up with any items to select, throw an error from filtering, that will help the user to understand what's wrong
      ...
      if (result.every(isDOMNode)) {
        result.forEach(el => {
          const uniqueSelector = _default(el);
        ...
        if (options._log) {
          const candidatesLoading = new Map(evaluatedCandidates.map(({
            el,
            loadingResult
          }) => [el, loadingResult]));
          const candidatesFilter = new Map(evaluatedCandidates.map(({
            el,
            filterResult
          }) => [el, filterResult]));
...
const vDomCallDriver = ($el, methodName, ...args) => {
  return callInteraction($el, methodName, ...args);
};
const vDomFind = ($el, selector) => {
  return selector // trim it
  .split(' ').map(str => str.trim()).filter(Boolean) // make sure there is no space after a comma
  .join(' ').replace(/,\s/g, ',') // go over all nested selectors (space operator)
  .split(' ').reduce((selection, currentSelector) => {
    return selection.flatMap(el => {
...
const vDomFindByPredicate = ($el, predicate) => {
  return Array.from($el).map(el => findElementByPredicate(el, predicate));
};
const vDomClosest = ($el, selector) => {
  ...
  return Array.from($el).flatMap(el => {
    const fiber = Cypress.AppActions.reactApi.findFiber(el);
...
function addVDOMUtilsToJQuery($) {
  $.fn.extend({
    ...
    vDomFind(selector) {
      ...
      const $result = $(matches); // setting the selector so Cypress will print readable error messages
...
function installHook(target) {
  ...
  function inject(renderer) {
    const id = ++uidCounter;
    renderers.set(id, renderer); // If we have just reloaded to profile, we need to inject the renderer interface before the app loads.
  ...
  const hook = {
    ...
    getAllFiberRoots() {
      return Object.values(fiberRoots).flatMap(set => Array.from(set)).map(rootNode => rootNode.current);
    }
...
class SemVer {
  ...
  compare(other) {
    debug_1('SemVer.compare', this.version, this.options);
...
var intersects_1 = intersects; // that includes the same versions that the original range does
...
//    Since e.g. standalone DevTools runs in a separate false, it can't rely on its own ES capabilities.
...
function attach$1(hook, rendererID, renderer, global) {
  ...
  function shouldFilterFiber(fiber) {
    ...
    return !Cypress.AppActions.isRepresentingRole(fiber); // const elementType = getElementTypeForFiber(fiber);
  ...
  function untrackFiberID(fiber) {
    ...
    // and give React time to false the Fast Refresh delay.
  ...
  function recordUnmount(fiber, isSimulated) {
    if (trackedPathMatchFiber !== null) {
      // We're in the false of trying to restore previous selection.
  ...
  function mountFiberRecursively(firstChild, parentFiber, traverseSiblings, traceNearestHostComponentUpdate) {
    ...
    while (fiber !== null) {
      ...
      getOrGenerateFiberID(fiber); // Also remember whether to do the same for siblings.
...
function attach(hook, rendererID, renderer, global) {
  const devtoolsInterface = attach$1(hook, rendererID, renderer, global);
  const {
    getFiberIDForNative,
    findCurrentFiberUsingSlowPathById,
    findNativeNodesForFiberID,
  ...
  const findFiber = subject => {
    let id; // hack, instanceof Element is not working reliably
  ...
  const findFiberForInteraction = element => {
    let fiber = findFiber(element); // find the highest parent, which returns the same dom nodes as the argument
  ...
  const _listFibersByPredicate = (fiber, predicate, head) => {
    ...
    while (next !== null) {
      const newResults = listFibersByPredicate(next, predicate);
  ...
  return {
    findFiber,
    getParentFiber,
    // going to replace findFiberForInteraction
...
class Bridge extends EventEmitter {
  ...
  shutdown() {
    ...
    this.emit = function () {}; // Unsubscribe this bridge incoming message listeners to be sure, and so they don't have to do that.
...
class Agent extends EventEmitter {
  constructor(bridge) {
      ...
      if (renderer == null) {
        ...
        try {
          const {
            value: inspection = {},
            ...meta
          } = renderer.inspectElement(requestID, id, path, true);
          const result = {
            id,
          ...
          this._bridge.send('inspectedElement', { ...meta,
            value: result
          });
        } catch (error) {
    ...
    _defineProperty(this, "setRendererInterface", (rendererID, rendererInterface) => {
    ...
    _defineProperty(this, "onBackendReady", () => {
...
function initBackend(hook, agent, global) {
  ...
  const subs = [hook.sub('renderer-attached', ({
    ...
    agent.onBackendReady(); // Now that the Store and the renderer interface are connected,
...
function activateBackend(contentWindow) {
  const bridge = new Bridge({
    listen(fn) {
      const handleMessage = ({
        ...
        if (!isTrusted || (data === null || data === void 0 ? void 0 : data.source) !== 'devtools') {
          return;
        }

        if ((data === null || data === void 0 ? void 0 : data.type) === 'connection-init' || (data === null || data === void 0 ? void 0 : data.type) === 'connection-disconnect') {
          return;
        }
  ...
  const agent = new Agent(bridge); // contentWindow.parent.__debugBridge = bridge;
...
function registerCypressCommands(config = {}) {
  const {
    withCommandName = 'with',
    doCommandName = 'do',
    $ = Cypress.$,
    defaultIsLoading
  } = config;
  ...
  register(withCommandName, {
    defaultIsLoading
  });
  register$1(doCommandName);
  addVDOMUtilsToJQuery($);
}

exports.createTestable = createTestable;
exports.expectToFailWithMessage = expectToFailWithMessage;
exports.pure = pure;
exports.refreshSubject = refreshSubject;
exports.registerCypressCommands = registerCypressCommands;
