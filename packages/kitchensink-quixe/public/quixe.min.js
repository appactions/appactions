...
catch(ex){qstackdump();Glk.fatal_error("Quixe init: "+show_exception(ex));debugger;if(opt_rethrow_exceptions)
throw ex;}}
...
msg+=")";}
qlog(msg);throw(msg);}
function make_code(val,funcname,arg){var func;if(funcname===undefined)
funcname='_func';if(arg===undefined)
...
context.code.push("if (leave_function()) return VMStopped;");context.code.push("pop_callstub("+val+");");}
...
else{oputil_unload_offstate(context,!unconditional);context.code.push("if (("+operand+")==0 || ("+operand+")==1) {");context.code.push("if (leave_function()) return VMStopped;");context.code.push("pop_callstub("+operand+");");context.code.push("}");context.code.push("else {");context.code.push("pc = ("+context.cp+"+("+operand+")-2) >>>0;");context.code.push("}");}
...
context.code.push("if (leave_function()) return VMStopped;");context.code.push("enter_function("+operands[0]+", "+operands[1]+");");context.code.push("return;");context.path_ends=true;},0x160:function(context,operands){oputil_unload_offstate(context);oputil_push_callstub(context,operands[1]);context.code.push("enter_function("+operands[0]+", 0);");context.code.push("return;");context.path_ends=true;},0x161:function(context,operands){oputil_unload_offstate(context);context.code.push("tempcallargs[0]=("+operands[1]+");");oputil_push_callstub(context,operands[2]);context.code.push("enter_function("+operands[0]+", 1);");context.code.push("return;");context.path_ends=true;},0x162:function(context,operands){oputil_unload_offstate(context);context.code.push("tempcallargs[0]=("+operands[1]+");");context.code.push("tempcallargs[1]=("+operands[2]+");");oputil_push_callstub(context,operands[3]);context.code.push("enter_function("+operands[0]+", 2);");context.code.push("return;");context.path_ends=true;},0x163:function(context,operands){oputil_unload_offstate(context);context.code.push("tempcallargs[0]=("+operands[1]+");");context.code.push("tempcallargs[1]=("+operands[2]+");");context.code.push("tempcallargs[2]=("+operands[3]+");");oputil_push_callstub(context,operands[4]);context.code.push("enter_function("+operands[0]+", 3);");context.code.push("return;");context.path_ends=true;},0x31:function(context,operands){context.offstack.length=0;context.offloc.length=0;context.offlocdirty.length=0;context.code.push("if (leave_function()) return VMStopped;");context.code.push("pop_callstub("+operands[0]+");");context.code.push("return;");context.path_ends=true;},0x32:function(context,operands){oputil_unload_offstate(context);oputil_push_callstub(context,operands[0]);context.code.push("store_operand("+operands[0]+",frame.framestart+frame.framelen+4*frame.valstack.length);");oputil_perform_jump(context,operands[1],true);context.path_ends=true;},0x33:function(context,operands){context.offstack.length=0;context.offloc.length=0;context.offlocdirty.length=0;context.code.push("pop_stack_to("+operands[1]+");");context.code.push("pop_callstub("+operands[0]+");");context.code.push("return;");context.path_ends=true;},0x40:function(context,operands){oputil_store(context,operands[1],operands[0]);},0x41:function(context,operands){oputil_store(context,operands[1],operands[0]);},0x42:function(context,operands){oputil_store(context,operands[1],operands[0]);},0x44:function(context,operands){var val;if(quot_isconstant(operands[0])){val=Number(operands[0]);val=(val&0x8000)?((val|0xffff0000)>>>0):(val&0xffff);context.code.push(operands[1]+val+");");}
...
var VMStopped={dummy:'The top-level function has returned.'};function leave_function(){var olddepth=frame.depth;stack.pop();if(stack.length==0){frame=null;return true;}
...
total_path_calls++;var res=path();if(res===VMStopped){done_executing=true;vm_stopped=true;}}
