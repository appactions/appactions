...
describe('Retrying', () => {
    it("with(appears delayed<spinner>) -- retry by testable's wait logic", () => {
        ...
        cy.with(Table)
            .do(Table.getColumnOrThrow('Fruit'))
    ...
    it('with(not existing) -- should timeout', () => {
        cy.visit('/instant');
        cy.with({ role: 'NotExisting', isTestable: true }).should('exist');
    ...
    it('with(not existing, "foo", "bar", "baz") -- should show the picker data in the exception', () => {
        ...
        const NotExisting = { role: 'NotExisting', isTestable: true, customPicker: () => () => true };
    ...
    it('<anything>.do(interaction<advancedPurityComposition>) -- advanced side-effect handling', () => {
        cy.visit('/');
        cy.get('[data-test="page-index"]').do(Table.advancedPurityComposition());
